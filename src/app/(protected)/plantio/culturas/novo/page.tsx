...
export default function NewCropPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess("Cultura cadastrada com sucesso.")
      setName("")

      setTimeout(() => {
        router.push("/plantio/culturas")
      }, 1000)
    } catch (err: any) {
      setError(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Cadastre uma nova cultura agrícola no sistema.
        </p>
      </div>
        ...
        <div className="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                type="button"
                onClick={() => router.push("/plantio/culturas")}
              ...
              >
                {loading ? "Salvando..." : "Cadastrar Cultura"}
              </button>
            </div>
          </form>
        </div>
